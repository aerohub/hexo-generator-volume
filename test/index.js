...
describe('Volume generator', function() {
  ...
  before(function() {
    return Post.insert([
      ...
      return posts[0].setVolumes(['foo']).then(function() {
        return posts[1].setVolumes(['bar']);
      }).then(function() {
        return posts[2].setVolumes(['foo']);
      }).then(function() {
        return posts[3].setVolumes(['foo']);
      });
    }).then(function() {
  ...
  it('pagination enabled', function() {
    hexo.config.volume_generator.per_page = 2;
    ...
    for (var i = 0, len = result.length; i < len; i++) {
      result[i].layout.should.eql(['volume', 'archive', 'index']);
    }

    result[0].path.should.eql('volumes/foo/');
    result[0].data.base.should.eql('volumes/foo/');
    result[0].data.total.should.eql(2);
    result[0].data.current.should.eql(1);
    result[0].data.current_url.should.eql('volumes/foo/');
    ...
    result[0].data.next_link.should.eql('volumes/foo/page/2/');
    result[0].data.volume.should.eql('foo');

    result[1].path.should.eql('volumes/foo/page/2/');
    result[1].data.base.should.eql('volumes/foo/');
    result[1].data.total.should.eql(2);
    result[1].data.current.should.eql(2);
    result[1].data.current_url.should.eql('volumes/foo/page/2/');
    result[1].data.posts.eq(0)._id.should.eql(posts[2]._id);
    result[1].data.prev.should.eql(1);
    result[1].data.prev_link.should.eql('volumes/foo/');
    result[1].data.next.should.eql(0);
    result[1].data.next_link.should.eql('');
    result[1].data.volume.should.eql('foo');

    result[2].path.should.eql('volumes/bar/');
    result[2].data.base.should.eql('volumes/bar/');
    result[2].data.total.should.eql(1);
    result[2].data.current.should.eql(1);
    result[2].data.current_url.should.eql('volumes/bar/');
    ...
    result[2].data.volume.should.eql('bar');

    // Restore config
    hexo.config.volume_generator.per_page = 10;
  });

  it('pagination disabled', function() {
    hexo.config.volume_generator.per_page = 0;
  ...
  it('custom pagination_dir', function() {
    ...
    result.map(function(item) {
      return item.path;
    }).should.eql(['volumes/foo/', 'volumes/foo/yo/2/', 'volumes/bar/']);
